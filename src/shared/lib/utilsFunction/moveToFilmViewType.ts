  ...
  return {
    ...
    actors:
      filmApi.persons
        ...
        .map((item) => {
          return { ...item, photoUrl: item.photo || undefined, _id: item.id };
        }) || undefined,
    filmCrew:
      filmApi.persons
        ?.filter((item) => item.enProfession !== 'actor')
    ...
    similarMovies: filmApi.similarMovies?.map((item) => {
      return {
        ...item,
        _id: item.id.toString(),
        title: item.name,
        rating: Object.values(item.rating).find((item) => item && item > 0) || undefined,
      };
    }),
