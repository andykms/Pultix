...
export const FilmAboutDetail: React.FC<FilmAboutDetailProps> = forwardRef(
  (
    ...
    const movieLengthString =
      hours && hours ? `${hours > 0 ? `${hours} ч ` : ''}${minutes} мин` : undefined;
        ...
        <div className={styles.filmInfo}>
          {raiting && (
            <div className={styles.raitingContainer}>
              <span className='title'>Рейтинг</span>
              <CriticInfo title='' value={raiting}></CriticInfo>
            </div>
          )}
          {year && <BlockInfo title='Год' blocks={[{ title: year.toString() }]}></BlockInfo>}
          {genres && (
            <BlockInfo
              title={'Жанры'}
          ...
          {movieLengthString && (
            <BlockInfo title='Длительность' blocks={[{ title: movieLengthString }]}></BlockInfo>
          )}
          {ageRaiting && (
            <BlockInfo
              title='Ограничение'
