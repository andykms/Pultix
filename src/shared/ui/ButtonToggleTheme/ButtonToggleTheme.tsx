...
export const ButtonToggleTheme = () => {
  ...
  return (
    <div className={clsx(styles.theme, isOpened && styles.opened)}>
      {isOpened ? (
        <ButtonUI type='opacity' width='50px' onClick={onClickBottom}>
          {theme === 'dark' ? lightSvg : darkSvg}
        </ButtonUI>
      ) : null}
      {!isOpened ? (
        <ButtonUI type='opacity' width='50px' onClick={onClickTop}>
          {theme === 'dark' ? darkSvg : lightSvg}
        </ButtonUI>
      ) : (
        <ButtonClose onClose={() => setIsOpened(false)}></ButtonClose>
      )}
    </div>
  );
};
