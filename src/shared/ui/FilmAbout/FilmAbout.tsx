...
export const FilmAbout: React.FC<FilmAboutProps> = (props: FilmAboutProps) => {
  const { title, raiting, year, description, genres, movieLength, ageRaiting, countries } = props;
  ...
  const movieLengthString =
    hours && minutes ? `${hours > 0 ? `${hours} ч ` : ''}${minutes} мин` : undefined;
          ...
          {countries && (
            <div className={styles.countriesContainer}>
              {countries.map((country) => {
                return <span className={clsx('description', styles.genre)}>{country}</span>;
              })}
            </div>
          )}
